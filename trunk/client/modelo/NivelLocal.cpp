...
void NivelLocal::generarPajaro() {
	...
	if (tipoPajaro != NO_PAJARO) {
		...
		switch (tipoPajaro) {
		case PAJARO_AZUL:
			escenario->lanzarPajaroAzul(Punto2D(posX, posY),
					Velocidad2D(velX, velY));
			break;
		case PAJARO_ROJO:
			escenario->lanzarPajaroRojo(Punto2D(posX, posY),
					Velocidad2D(velX, velY));
			break;
		case PAJARO_VERDE:
			escenario->lanzarPajaroVerde(Punto2D(posX, posY),
					Velocidad2D(velX, velY));
			break;
		}
	}
